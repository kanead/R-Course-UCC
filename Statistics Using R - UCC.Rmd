...
For convenience, the above distribution is transformed into the standard normal distribution by using the z-score of the sample mean. So the z score for our sample mean is our sample mean minus the mean under the null hypothesis divided by the standard error:
...
To reiterate, in statistics we use 'Null distributions' which are the sampling distributions of our **statistics** under the null hypothesis. 
...
Let's assume a simple experimental design where we have an independent variable with either vaccine or placebo and a dependent variable which is the rate of polio.

This will give us two means that can be compared using a t-test. NHST can be conducted, yielding a p-value. 
...
If not, then the homogeneity of variance assumption is violated. Simulations indicate this results in an increased probability of tpe 1 error.
...
If it is violated we can adjust the df and p-value using Welch's procedure, which can protect against type 1 error, or we can use a non-parametric test.
...
All sample statistics e.g. a sample mean, are point estimates i.e. one point from a sample distribution. More specifically, a sample mean represents a single point in a sampling distribution. Any one sample will never be perfect. The logic of confidence intervals is to report a range of values, rather than a single value. In other words, report an interval estimate rather than a point estimate. 

The main argument for interval estimates is the reality of sampling error. Sampling error implies that point estimates will vary from one study to the next (as we saw above we use standard error to get a measure of sampling error). A researcher will therefore be more confident about accuracy with an interval estimate. How frequently the observed interval contains the true parameter if the experiment is repeated is called the confidence level.

The general form of a confidence interval is: point estimate $\pm$ margin of error, where the margin of error is Z * the standard error of the point estimate

### Calculating Confidence Intervals around sample means
Remember in this case, this is the sampling distribution of the sample means, not the distribution of the raw data. So what we're saying is that 95% of all sample means are between the shaded area. The remaining area in the tails is called the $\alpha$ and here it is 5%. We take this $\alpha$ value and divide it evenly among both tails so that we have 2.5% of 0.025. Recall that for the standard normal distribution there is a 95% probability that a standard normal variable, Z, will fall between -1.96 and 1.96. 
...
The 95% confidence interval for the population mean, can also be expressed as:

\[\bar{X}\pm 1.96 \frac{\sigma}{\sqrt{n}}\]

What the equation tells us is that we have a mean $\pm$ 1.96 standard errors (standard deviations)

Why are we using standard error of the mean here? Because it is the standard deviation of the sampling distribution. 
...
Calculating the confidence interval when we don't know the population standard deviation is similar to using a z-distribution. The only difference is that we use the t-distribution rather than the z-distribution. Here we repeat the procedures above, but we will assume that we are working with a sample standard deviation rather than an exact standard deviation.
...
95 % confidence is a confidence that in the long-run 95 % of the confidence intervals will include the population mean. It is a confidence in the algorithm and not a statement about a single confidence interval.

Thus it is not correct to say that your interval has a 95% chance of containing the population parameter. It either does or it doesn't.
